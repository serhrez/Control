...
class PredefinedRealm {
    static func populateRealm(_ realm: Realm) {
        ...
        try realm.write {
            ...
            task.date = RlmTaskDate(date: Date(), reminder: .fiveMinutesEarly, repeat: .monthly)
            task.priority = .high
            project1.tasks.append(task)
            ...
            task.tags.append(tag1)
            task.priority = .low
            task.date = .init(date: datereferencex, reminder: .tenMinutesEarly, repeat: nil)
            project2.tasks.append(task)
            task = RlmTask(name: "Task2", priority: .medium, isDone: true)
            task.priority = .high
            task.tags.append(tag2)
            task.date = .init(date: datereferencex, reminder: .thirtyMinutesEarly, repeat: nil)
            project2.tasks.append(task)
            ...
            task.date = .init(date: datereferencex, reminder: .fiveMinutesEarly, repeat: nil)
